...
Manages a named `config interface` section in `/etc/config/network` through HTTP ubus UCI operations.

## Example Usage

```hcl
resource "openwrt_network" "runner" {
  name      = "vlan20"
  device    = "br-vlan20"
  proto     = "static"
...
- `device` (Required) Network device assigned to the network.
...
- `dns` (Optional) DNS server list attached to this network.

## Attributes Reference

- `id` Canonical network name.
...
Import using the network name:

```bash
terraform import openwrt_network.runner vlan20
```
